...
# THE LEGACY DELETION, ACTUALLY RUN.
#
...
class LegacyDeletionTest < ActiveSupport::TestCase
  ...
  EDITS = [
    # The one line in the canonical entry point that goes with the legacy trees.
    ...
    # thing that checks its post-deletion shape at all.
  ...
  test "every canonical entry point loads and keeps its surface after the legacy deletion" do
  ...
  test "every canonical entry point defines its advertised constants after the legacy deletion" do
    in_deleted_tree do |tree|
  ...
  # Yields the path to a lib/ with the legacy deletion applied.
  def in_deleted_tree
    Dir.mktmpdir("snapdiff_deleted") do |dir|
      ...
      DELETED.each do |path|
        ...
        assert File.exist?(target), "deletion set names #{path}, which does not exist"
      ...
      EDITS.each do |file, line, replacement|
        ...
        assert_includes source, line, "deletion edit for #{file} no longer matches the file"
