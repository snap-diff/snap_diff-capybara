...
class DiffTest < ActiveSupport::TestCase
  ...
  teardown do
    ...
    SnapDiff.config.screenshot_format = @orig_screenshot_format
  ...
  test "writes screenshot to alternate save path" do
    default_path = SnapDiff.config.save_path
    SnapDiff.config.save_path = "foo/bar"
    ...
    FileUtils.remove_entry SnapDiff.config.screenshot_area_abs
    SnapDiff.config.save_path = default_path
  end

  test "does not error when using stability_time_limit" do
    default_stability_time_limit = SnapDiff.config.stability_time_limit
    SnapDiff.config.stability_time_limit = 0.001

    screenshot "a"
  ensure
    SnapDiff.config.stability_time_limit = default_stability_time_limit
  end
  ...
  test "raises error on teardown for non-Minitest" do
    ...
    assert_raises(SnapDiff::ExpectationNotMet, expected_message) { test_case.teardown }
    assert_empty(SnapDiff.session.assertions)
  end
  ...
  def screenshot_dir
    File.join(SnapDiff.config.screenshot_area, *screenshot_namer.directory_parts)
  end
  ...
  def build_full_name(name)
    SnapDiff.session.screenshot_namer.full_name(name)
  end
end
