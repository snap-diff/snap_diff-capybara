...
module SnapDiff
  module Drivers
    class VipsDriverTest < ActiveSupport::TestCase
      ...
      #
      # This was latent while chunky_png existed: a Comparison built without an
      # explicit driver defaulted to chunky_png, which re-read the file every
      # time. 2.1 made vips the only backend, so the stale read became the only
      # behaviour -- it turned identical images into "different" and vice versa.
      #
      ...
      test "#from_file re-reads a path that was overwritten within the same second" do
        ...
        first = driver.from_file(path)
        first_avg = first.avg # force evaluation BEFORE the overwrite
